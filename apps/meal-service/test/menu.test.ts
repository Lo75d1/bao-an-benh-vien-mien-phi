...
import { createMenuSnapshot, evaluateMenu, parseMenuItems, type MenuItemInput } from "../src/lib/menu-logic";
...
test("snapshot là bản sao bất biến với thay đổi dữ liệu đầu vào sau đó", () => { const source = [{ ...item, dishName: "Cơm chính", nutrients: { ...complete } }]; const snapshot = createMenuSnapshot(source); source[0].grams = 250; source[0].itemName = "Cơm đã sửa"; source[0].dishName = "Tên đã sửa"; assert.deepEqual(snapshot.items[0], { foodId: "food-1", itemName: "Cơm", dishName: "Cơm chính", grams: 100, wastePercent: 2 }); });
test("snapshot cũ không có tên món vẫn đọc được", () => { const items = parseMenuItems({ version: 1, items: [{ foodId: "food-1", itemName: "Cơm", grams: 100, wastePercent: 2 }] }); assert.equal(items[0]?.dishName, "Món 1"); });
