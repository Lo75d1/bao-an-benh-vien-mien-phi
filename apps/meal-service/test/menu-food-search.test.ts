...
import { nutritionSourceTitle } from "../src/lib/i18n";
...
test("source tooltip labels are localized and badge-only tokens stay canonical", () => {
  assert.equal(nutritionSourceTitle("VDD", "vi"), "Nguồn dữ liệu: VDD");
  assert.equal(nutritionSourceTitle("RNI + VDD", "en"), "Data sources: RNI, VDD");
  assert.equal(nutritionSourceTitle("RNI + USDA", "en"), "Data sources: RNI, USDA");
});
