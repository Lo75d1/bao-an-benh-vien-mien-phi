...
export function AppShell({ user, children, workflowStatus, adminNotifications = [], demoClock }: { user: SessionUser; children: ReactNode; workflowStatus?: ReactNode; adminNotifications?: Array<{ id: string; label: string; detail: string }>; demoClock?: { nowIso: string; simulated: boolean } }) {
  const pathname = usePathname();
  const branding = useBranding();
  const inheritedDemoClock = useDemoClock();
  const activeDemoClock = demoClock ?? inheritedDemoClock ?? undefined;
  const operationsHome = activeDemoClock ? "/demo" : user.role === "ADMIN" || user.role === "DIETITIAN" ? "/quan-ly" : "/";
  const seesOperationsNotifications = true;
  return <div className="app-frame"><a href="#main-content" className="skip-link">Đi tới nội dung chính</a><header className="top-shell" style={{ backgroundColor: "var(--brand-surface)", color: "var(--brand-foreground)" }}><Link href={operationsHome} className="top-brand"><span>{branding.logoDataUrl ? <Image src={branding.logoDataUrl} alt="" width={38} height={38} unoptimized/> : branding.shortName}</span><strong>{branding.organizationName}</strong></Link><nav aria-label="Điều hướng chính">{NAVIGATION[user.role].map((item) => { const active = pathname.startsWith(item.href); return <Link data-demo-nav={item.href} key={item.href} href={item.href} aria-current={active ? "page" : undefined}>{item.label}</Link>; })}</nav><div className="top-shell-actions">{activeDemoClock ? <span className="demo-system-label">DEMO</span> : null}{activeDemoClock ? <a className="top-project-link" href="https://dinhduong2598.food" target="_blank" rel="noreferrer"><span>Dinh dưỡng 2598</span><ExternalLink aria-hidden="true"/></a> : null}{activeDemoClock ? <DemoClockControl {...activeDemoClock}/> : null}{workflowStatus ? <div className="top-workflow-status">{workflowStatus}</div> : null}{seesOperationsNotifications && <DropdownMenu><DropdownMenuTrigger asChild><button type="button" className={adminNotifications.length ? "top-notifications attention" : "top-notifications"} aria-label={`${adminNotifications.length} thông báo vận hành`}><Bell aria-hidden="true"/><span>{adminNotifications.length || "—"}</span></button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-80"><DropdownMenuLabel>Thông báo vận hành</DropdownMenuLabel><DropdownMenuSeparator/>{adminNotifications.length ? adminNotifications.map((item) => <DropdownMenuItem key={item.id} className="grid gap-0.5"><strong>{item.label}</strong><small className="text-muted-foreground">{item.detail}</small></DropdownMenuItem>) : <DropdownMenuItem disabled>Không có cảnh báo cần xử lý.</DropdownMenuItem>}</DropdownMenuContent></DropdownMenu>}<AccountMenu user={user}/></div></header><div id="main-content" tabIndex={-1} className="app-main">{children}</div>{activeDemoClock ? <DemoGuide user={user}/> : null}</div>;
}
