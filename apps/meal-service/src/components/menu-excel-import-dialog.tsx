...
export function MenuExcelImportDialog({ meals, mealName, onApply }: { meals: MealOption[]; mealName: string; onApply: (rows: Array<{ mealId: string; item: MenuItemInput }>) => void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button type="button" data-demo-guide="excel-import"><FileSpreadsheet/>Nhập Excel</button></DialogTrigger><DialogContent className="nutrition-import-dialog max-h-[92vh] max-w-6xl overflow-hidden"><DialogHeader><DialogTitle>Nhập thực đơn từ Excel</DialogTitle><DialogDescription>Đọc từng hàng thực phẩm, kiểm tra các chất dinh dưỡng và chọn đúng mã trước khi nhập. Dữ liệu hiện tại chưa bị thay đổi.</DialogDescription></DialogHeader>
    {!headers.length ? <div className="nutrition-import-start"><button type="button" className="nutrition-import-drop" onClick={() => inputRef.current?.click()}><Upload/><strong>Chọn tệp Excel</strong><span>.xlsx hoặc .xlsm · hàng đầu là tiêu đề · mỗi thực phẩm là một hàng</span></button><button type="button" className="nutrition-import-template" onClick={downloadTemplate}><Download/> Tải file mẫu đúng các trường</button></div> : <div className="nutrition-import-body">
      ...
      <section className="nutrition-import-preview"><header><strong>Xem trước · {rows.length} hàng</strong><span>{status}</span></header><div><table><thead><tr><th>Dòng</th><th>Mã chế độ ăn</th><th>Bữa</th><th>Kiểu / món</th><th>Thực phẩm</th><th>g/suất</th><th>kcal</th><th>Đạm</th><th>Béo</th><th>Bột đường</th><th>Trạng thái</th></tr></thead><tbody>{rows.map((row, index) => { const meal = mealByCode.get(row.dietCode.toLocaleLowerCase("vi")); const warning = !meal ? "Chọn đúng mã" : meal.approved ? "Mã đã tự khóa" : row.warnings.filter((item) => item !== "Chưa chọn mã chế độ ăn").join(" · "); return <tr key={row.rowNumber} className={warning ? "has-warning" : ""}><td>{row.rowNumber}</td><td><select value={meal?.code ?? ""} onChange={(event) => patchRow(index, { dietCode: event.target.value })}><option value="">— Chọn mã —</option>{meals.map((option) => <option key={option.id} value={option.code} disabled={option.approved}>{option.code}{option.approved ? " · đã tự khóa" : ""}</option>)}</select></td><td>{row.mealName || "—"}</td><td><input value={row.dishName} onChange={(event) => patchRow(index, { dishName: event.target.value })}/></td><td><input value={row.foodName} onChange={(event) => patchRow(index, { foodName: event.target.value })}/></td><td><input type="number" min="0.1" step="0.1" value={row.grams ?? ""} onChange={(event) => patchRow(index, { grams: event.target.value ? Number(event.target.value) : null })}/></td><td>{row.nutrients.energyKcal ?? "—"}</td><td>{row.nutrients.proteinG ?? "—"}</td><td>{row.nutrients.lipidG ?? "—"}</td><td>{row.nutrients.glucidG ?? "—"}</td><td>{warning || "Sẵn sàng"}</td></tr>; })}</tbody></table></div></section>
    </div>}
