...
export function MenuEditor({ dietMeal, foods, thresholds, templates, copies, approveAction, saveTemplateAction }: { dietMeal: { id: string; dietTypeId: string; feedingRoute: "NORMAL" | "SONDE"; approved?: boolean; existing: SourceItem[] }; foods: FoodOption[]; thresholds: DietCodeThresholds | null; templates: Source[]; copies: Source[]; approveAction: (data: FormData) => void; saveTemplateAction: (data: FormData) => void }) {
  ...
  const dishes = useMemo(() => [...new Set(items.map((item) => item.dishName || "Món 1"))], [items]);
  ...
  return <form className="menu-workbench ration-workbench">
    ...
    <div className="ration-layout">
      ...
      <section className="ration-entry" aria-labelledby="ration-entry-title"><div className="panel-title"><div><p className="eyebrow">Đang nhập vào</p><h2 id="ration-entry-title">{activeDish}</h2></div>{items.some((item) => (item.dishName || "Món 1") === activeDish) && <button type="button" className="remove-button" onClick={() => removeDish(activeDish)}>Xóa món</button>}</div>
        <div className="food-finder"><label htmlFor="menu-food-search">Tìm thực phẩm</label><input id="menu-food-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="VD: cá chép, sữa chua; gõ không dấu được" autoComplete="off"/>{query.trim() && <div className="food-results">{visibleFoods.length ? visibleFoods.map((food) => <button type="button" key={food.id} onClick={() => addFood(food)}><span><strong>{food.name}</strong><small>{food.wastePercent === null ? "Chưa có tỷ lệ thải bỏ" : `Thải bỏ ${food.wastePercent}%`}</small></span><em>+ Thêm</em></button>) : <p>Không tìm thấy thực phẩm phù hợp.</p>}</div>}</div>
        ...
        {!items.some((item) => (item.dishName || "Món 1") === activeDish) && <div className="menu-empty"><strong>Chưa có thực phẩm trong {activeDish}</strong><span>Tìm thực phẩm phía trên để thêm; dinh dưỡng tính trên gram sống sạch.</span></div>}
      </section>
      <aside className="ration-analysis"><div className="panel-title"><div><p className="eyebrow">Tính tức thời</p><h2>Tổng khẩu phần</h2></div><span>{items.length || "—"} dòng</span></div><dl className="macro-summary"><div><dt>Năng lượng</dt><dd>{totals.energyKcal === null ? "—" : number.format(totals.energyKcal)} <small>kcal</small></dd></div><div><dt>Đạm</dt><dd>{totals.proteinG === null ? "—" : number.format(totals.proteinG)} <small>g</small></dd></div><div><dt>Béo</dt><dd>{totals.lipidG === null ? "—" : number.format(totals.lipidG)} <small>g</small></dd></div><div><dt>Bột đường</dt><dd>{totals.glucidG === null ? "—" : number.format(totals.glucidG)} <small>g</small></dd></div></dl><DietEvaluation criteria={evaluation.criteria}/></aside>
    </div>
