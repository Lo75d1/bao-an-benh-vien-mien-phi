...
import { nutritionSourceTitle, type Language } from "@/lib/i18n";

function SourceBadge({ source, language }: { source?: FoodResult["sourceBadge"]; language: Language }) {
  return source ? <em className="source-badge-2598" title={nutritionSourceTitle(source, language)}>{source}</em> : null;
}
...
export function MenuFoodSearch({ kind, onPickFood, onPickDish, placeholder, filterIconOnly = false, inputRef, language = "vi" }: { kind: "food" | "dish"; onPickFood?: (food: FoodResult) => void; onPickDish?: (dish: DishResult) => void; placeholder?: string; filterIconOnly?: boolean; inputRef?: Ref<HTMLInputElement>; language?: Language }) {
  ...
  return <div className="search-2598">
    ...
    {q.trim() && <div className="search-2598-results">{loading ? <p>{en ? "Searching…" : "Đang tìm…"}</p> : failed ? <p>{en ? "Unable to connect to search data." : "Không kết nối được dữ liệu tra cứu."}</p> : items.length === 0 ? <p>{en ? "No results found." : "Không tìm thấy kết quả."}</p> : items.map((item) => kind === "food" ? <button type="button" key={item.id} onClick={() => { onPickFood?.(item as FoodResult); setQ(""); }}><span><strong>{item.name}<SourceBadge source={(item as FoodResult).sourceBadge} language={language}/></strong><small>{(item as FoodResult).source || (en ? "Source not recorded" : "Chưa ghi nguồn")} · {(item as FoodResult).energyKcal ?? "—"} kcal/100g</small></span><b>＋ {en ? "Add" : "Thêm"}</b></button> : <button type="button" key={item.id} onClick={() => { onPickDish?.(item as DishResult); setQ(""); }}><span><strong>{item.name}<SourceBadge source={(item as DishResult).sourceBadge} language={language}/></strong><small>{(item as DishResult).ingredients.filter((ingredient) => ingredient.food).length}/{(item as DishResult).ingredients.length} {en ? "ingredients with data" : "nguyên liệu có dữ liệu"}</small></span><b>＋ {en ? "Add dish" : "Thêm món"}</b></button>)}</div>}
  </div>;
}
