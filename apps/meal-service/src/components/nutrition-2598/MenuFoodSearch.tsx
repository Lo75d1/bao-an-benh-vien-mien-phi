...
export function MenuFoodSearch({ kind, onPickFood, onPickDish, placeholder }: { kind: "food" | "dish"; onPickFood?: (food: FoodResult) => void; onPickDish?: (dish: DishResult) => void; placeholder?: string }) {
  ...
  return <div className="search-2598">
    <div className="search-2598-row"><input value={q} onChange={(event) => { setQ(event.target.value); if (!event.target.value.trim()) { setItems([]); setFailed(false); } }} placeholder={placeholder ?? (kind === "food" ? "Tìm thực phẩm để thêm vào món…" : "Tìm món để thêm cả công thức…")}/><button type="button" onClick={() => setFiltersOpen((open) => !open)}>⚙ Bộ lọc</button></div>
