...
export function MenuFoodSearch({ kind, onPickFood, onPickDish, placeholder, filterIconOnly = false, inputRef }: { kind: "food" | "dish"; onPickFood?: (food: FoodResult) => void; onPickDish?: (dish: DishResult) => void; placeholder?: string; filterIconOnly?: boolean; inputRef?: Ref<HTMLInputElement> }) {
  ...
  return <div className="search-2598">
    <div className="search-2598-row"><input ref={inputRef} value={q} onChange={(event) => { setQ(event.target.value); if (!event.target.value.trim()) { setItems([]); setFailed(false); } }} placeholder={placeholder ?? (kind === "food" ? "Tìm thực phẩm để thêm vào món…" : "Tìm món để thêm cả công thức…")}/><button type="button" aria-label="Bộ lọc" title="Bộ lọc" onClick={() => setFiltersOpen((open) => !open)}>{filterIconOnly ? "⚙" : "⚙ Bộ lọc"}</button></div>
