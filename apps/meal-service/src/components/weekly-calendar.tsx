                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <td key={dayKey} className={`${dayKey === todayKey ? "calendar-today" : dayKey < todayKey ? "calendar-past" : "calendar-future"} ${state?.isCurrent ? "calendar-current-meal" : ""} ${state?.tone === "danger" ? "calendar-incomplete" : ""}`}>
                        {role === "DIETITIAN" && event?.dietMeals[0] ? <Link className="calendar-cell-button" href={`/thuc-don?meal=${encodeURIComponent(event.dietMeals[0].id)}`} aria-label={`Lên thực đơn ${mealType.name} ${formatVnDay(day.date)}`}>{content}</Link> : detail && state ? <MealDetailDialog meal={detail} date={formatVnDay(day.date)} stateLabel={state.label} trigger={<button type="button" className="calendar-cell-button" aria-label={`Xem chi tiết ${mealType.name} ${formatVnDay(day.date)}`}>{content}</button>}/> : content}
                      </td>
                    );
                  })}
                </tr>
