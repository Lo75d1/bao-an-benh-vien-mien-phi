...
import { DietName, EmptyState, StatusBadge } from "@/components/presentation";
      ...
      {mealTypes.length === 0 ? (
        ...
        <div className="calendar-scroll">
          <table className="calendar-table">
            <thead><tr><th scope="col">Bữa</th>{days.map((day) => <th scope="col" key={day.label}><time dateTime={toDateKey(day.date)}>{formatVnDay(day.date)}</time></th>)}</tr></thead>
            <tbody>
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <td key={toDateKey(day.date)}>
                        <div className="cell-status">{status ? <StatusBadge status={status}/> : <TooltipProvider><Tooltip><TooltipTrigger className="missing">—</TooltipTrigger><TooltipContent>Chưa có dữ liệu</TooltipContent></Tooltip></TooltipProvider>}</div>
