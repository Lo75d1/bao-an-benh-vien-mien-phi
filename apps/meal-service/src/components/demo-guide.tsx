...
export function DemoGuide({ user }: { user: SessionUser }) {
  ...
  return <aside className="demo-guide-card" aria-live="polite"><header><span>Hướng dẫn Demo · {index + 1}/{steps.length}</span><button type="button" onClick={close} aria-label="Thoát hướng dẫn"><X/></button></header><strong>{step.title}</strong><p>{step.body}</p>{step.time && clock ? <form action={updateDemoClockAction} className="demo-guide-time" onSubmit={() => sessionStorage.setItem(`demo-guide-index:${user.email}`, String(index))}><input type="hidden" name="returnTo" value={step.href}/><input type="hidden" name="mode" value="SET"/><input type="hidden" name="now" value={`${vietnamDate(clock.nowIso)}T${step.time}`}/><button type="submit"><Clock3/><span>{step.timeLabel}</span><strong>Tua tới {step.time}</strong></button></form> : null}<footer><span>{pathname === step.href ? step.target ? "Ô cần thao tác đang được làm nổi bật" : "Thực hiện trên màn này" : "Sẽ mở đúng màn hình ở bước tiếp"}</span>{index === steps.length - 1 && full && following ? <button type="button" onClick={changeRole} disabled={switching}>{switching ? <LoaderCircle className="animate-spin"/> : null}Sang {following.label}</button> : <button type="button" onClick={advance}>{index === steps.length - 1 ? "Hoàn tất" : "Tiếp theo"}</button>}</footer></aside>;
}
