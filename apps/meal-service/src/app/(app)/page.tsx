...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ diet?: string; date?: string; patient?: string; note?: string }> }) {
  ...
  return <main className="public-menu-home" style={publicStyle}>
    ...
    <section className="public-patient-feedback" id="gui-ghi-chu" aria-labelledby="public-note-title"><div className="public-patient-feedback-copy"><MessageSquareText aria-hidden="true"/><div><p className="eyebrow">Gửi tới khoa điều trị / quản trị</p><h2 id="public-note-title">Gửi ghi chú / phản ánh</h2><p>Nội dung này sẽ được gửi đến Khoa điều trị và Quản trị hệ thống. Ghi chú bữa ăn chỉ chuyển tới bếp sau khi được khoa hoặc Admin kiểm tra; không nhập thông tin bệnh án.</p></div></div>
      {query.note === "sent" ? <p className="patient-form-success" role="status">Đã gửi nội dung. Khoa điều trị và Quản trị hệ thống sẽ xem xét.</p> : null}
      {query.note && query.note !== "sent" ? <p className="patient-form-error" role="alert">{query.note === "limited" ? "Bạn đã gửi quá nhiều nội dung. Vui lòng thử lại sau." : query.note === "invalid" ? "Nội dung cần từ 3 đến 500 ký tự." : "Chưa thể gửi nội dung lúc này. Vui lòng thử lại sau."}</p> : null}
      {menu.departments.length ? <form action={submitPublicPatientNoteAction}><input type="hidden" name="returnDiet" value={menu.selectedDiet?.code ?? ""}/><input type="hidden" name="returnDate" value={menu.selectedDate}/><input type="hidden" name="mealDate" value={menu.selectedDate}/><fieldset className="patient-submission-type"><legend>Chọn loại nội dung</legend><label><input type="radio" name="type" value="MEAL_NOTE" defaultChecked/>Ghi chú bữa ăn</label><label><input type="radio" name="type" value="FEEDBACK"/>Phản ánh</label></fieldset><label>Khoa điều trị<select name="departmentToken" required defaultValue=""><option value="" disabled>Chọn tên khoa</option>{menu.departments.map((department) => <option key={department.id} value={department.token}>{department.name}</option>)}</select></label><label>Nội dung <span>bắt buộc</span><textarea name="note" minLength={3} maxLength={500} required placeholder="Ví dụ: Xin lưu ý món ăn cần mềm hơn hoặc phản ánh về bữa ăn…"/></label><label>Tên người gửi <span>không bắt buộc</span><input name="contactName" maxLength={100} autoComplete="name"/></label><label>Thông tin liên hệ <span>không bắt buộc</span><input name="contactInfo" maxLength={120} autoComplete="tel" placeholder="Số điện thoại hoặc buồng/phòng nếu muốn khoa phản hồi"/></label><button type="submit">Gửi nội dung</button></form> : <p className="public-note-unavailable">— · Bệnh viện chưa mở khoa nhận nội dung công khai.</p>}
    </section>
