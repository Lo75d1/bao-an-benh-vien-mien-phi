...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ diet?: string; date?: string; patient?: string; note?: string; lang?: string }> }) {
  ...
  return <main className="public-menu-home" style={publicStyle}>
    <PublicViewTracker/>
    <header className="public-menu-header"><a href="#public-menu-browser" className="public-menu-brand">{branding.logoDataUrl ? <Image src={branding.logoDataUrl} alt={`Logo ${branding.organizationName}`} width={40} height={40} unoptimized/> : <span>{branding.shortName}</span>}<strong>{branding.organizationName}</strong></a><div className="public-language-switch"><a href={languageHref("vi")} aria-current={language === "vi" ? "true" : undefined}>VI</a><span>|</span><a href={languageHref("en")} aria-current={language === "en" ? "true" : undefined}>EN</a></div><Dialog><DialogTrigger asChild><button type="button" className="staff-login-trigger"><LogIn aria-hidden="true"/>Đăng nhập nhân viên</button></DialogTrigger><DialogContent className="max-h-[92vh] max-w-md overflow-y-auto"><DialogHeader><DialogTitle>Đăng nhập nhân viên</DialogTitle><DialogDescription>Dùng tài khoản do bệnh viện cấp để vào khu vực làm việc.</DialogDescription></DialogHeader><LoginForm/></DialogContent></Dialog></header>
    ...
    <section className="public-patient-feedback" id="gui-ghi-chu" aria-labelledby="public-note-title"><div className="public-patient-feedback-copy"><MessageSquareText aria-hidden="true"/><div><p className="eyebrow">{t.public.sendToDepartment}</p><h2 id="public-note-title">{t.public.feedbackKitchenNote}</h2><p>{t.public.feedbackHelp}</p></div></div>
      ...
      {menu.departments.length ? <form action={submitPublicPatientNoteAction}><input type="hidden" name="returnDiet" value={menu.selectedDiet?.code ?? ""}/><input type="hidden" name="returnDate" value={menu.selectedDate}/><input type="hidden" name="returnLang" value={language}/><fieldset className="public-submission-type"><legend>{t.public.content}</legend><label><input type="radio" name="type" value="FEEDBACK" defaultChecked/> {t.public.feedback}</label><label><input type="radio" name="type" value="KITCHEN_NOTE"/> {t.public.kitchenNote}</label></fieldset><label>{t.public.department}<select name="departmentToken" required defaultValue=""><option value="" disabled>{t.public.chooseDepartment}</option>{menu.departments.map((department) => <option key={department.id} value={department.token}>{department.name}</option>)}</select></label><label>{t.public.content} <span>{t.public.required}</span><textarea name="note" minLength={3} maxLength={500} required placeholder="Ví dụ: suất ăn giao trễ, thiếu suất, lưu ý chuẩn bị…"/></label><label>{t.public.senderName} <span>{t.public.optional}</span><input name="contactName" maxLength={100} autoComplete="off"/></label><label>{t.public.contactInfo} <span>{t.public.optional}</span><input name="contactInfo" maxLength={120} autoComplete="off"/></label><button type="submit">{t.public.submit}</button></form> : <p className="public-note-unavailable">{t.public.noteUnavailable}</p>}
    </section>
