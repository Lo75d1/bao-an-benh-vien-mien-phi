...
export function LanguageForm({ current, action }: { current: Language; action: (formData: FormData) => Promise<void> }) {
  ...
  return <form className="grid gap-3" action={action}>
    <label className="grid gap-2 text-sm font-medium text-[#123c36]">Ngôn ngữ / Language
    ...
    <p className="text-sm leading-6 text-muted-foreground">Lựa chọn được lưu theo tài khoản và giữ sau khi đăng xuất/đăng nhập lại.</p>
    <button className="primary-action min-h-11" disabled={pending}>{pending ? "Đang lưu…" : "Lưu ngôn ngữ"}</button>
  </form>;
}
