...
import { createServingReport, normalizeReporterName, normalizeServingNote, requireNurseDepartment, type ServingLineInput } from "@/lib/serving-report";
...
export async function saveServingReportAction(formData: FormData) {
  ...
  await createServingReport({ mealEventId, departmentId, reportedByName: normalizeReporterName(formData.get("reportedByName")), lines }, user);
