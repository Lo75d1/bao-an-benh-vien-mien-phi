...
import { DeliveryReceiptControl, LateAdditionForm } from "./nurse-action-forms";
...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string; route?: string; demoNow?: string }> }) {
  ...
  const deliveryReceiptTrigger = event && report && (currentPhase === "SERVING" || currentPhase === "PASSED") ? <DeliveryReceiptControl eventId={event.id} eventName={event.mealType.name} route={data.route} expected={expectedReceiptQuantity} receipt={receipt ? { status: receipt.status, expectedQuantity: receipt.expectedQuantity, receivedQuantity: receipt.receivedQuantity, note: receipt.note, confirmedAt: receipt.confirmedAt.toISOString(), confirmedBy: receipt.confirmedBy.displayName } : null} action={confirmDeliveryReceiptAction}/> : null;
