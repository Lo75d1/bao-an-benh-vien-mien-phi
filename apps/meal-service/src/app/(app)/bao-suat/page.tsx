...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string; route?: string }> }) {
  ...
  const [data, pendingNotes] = await Promise.all([readNurseServingDay(user.id, requestedRoute), readPendingPatientNotes(user.id)]);
  const { saved } = params;
