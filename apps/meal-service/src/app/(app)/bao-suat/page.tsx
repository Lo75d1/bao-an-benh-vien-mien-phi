...
import { DeliveryReceiptForms, LateAdditionForm } from "./nurse-action-forms";
...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string; route?: string; demoNow?: string }> }) {
  ...
  return <AppShell user={user} adminNotifications={notifications} workflowStatus={currentEvent && currentPhase ? <span><strong>{currentEvent.mealType.name}</strong> — {MEAL_PHASE_LABEL[currentPhase]}</span> : null}><main className="nurse-report-page">
