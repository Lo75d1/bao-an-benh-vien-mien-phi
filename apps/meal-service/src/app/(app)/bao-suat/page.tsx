...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string; route?: string; meal?: string; demoNow?: string }> }) {
  ...
  const notesTrigger = <Dialog><DialogTrigger asChild><button type="button" className={pendingNotes.length ? "nurse-note-trigger has-notes" : "nurse-note-trigger"} aria-label={pendingNotes.length ? `${pendingNotes.length} ghi chú bệnh nhân chờ xác nhận` : "Không có ghi chú bệnh nhân"}><MessageCircle aria-hidden="true"/>{pendingNotes.length ? <span>{pendingNotes.length}</span> : null}</button></DialogTrigger><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>Ghi chú bệnh nhân</DialogTitle><DialogDescription>Chỉ ghi chú được xác nhận mới chuyển sang góc ghi chú của bếp.</DialogDescription></DialogHeader>{pendingNotes.length ? <div className="nurse-note-dialog-list">{pendingNotes.map((note) => <article key={note.id}><div><strong>{note.note}</strong><small>{note.department.name} · {dateLabel.format(note.mealDate)}{note.contactName ? ` · ${note.contactName}` : ""}</small></div><form action={reviewPatientNoteAction} onSubmit={(event) => event.stopPropagation()}><input type="hidden" name="noteId" value={note.id}/><input type="hidden" name="route" value={data.route}/><input name="reviewNote" maxLength={100} placeholder="Lý do nếu từ chối"/><button className="secondary-button" name="status" value="REJECTED">Từ chối</button><button className="primary-action" name="status" value="APPROVED">Xác nhận chuyển bếp</button></form></article>)}</div> : <p>— · Không có ghi chú chờ xác nhận.</p>}</DialogContent></Dialog>;
  ...
  const soundControl = <SoundNotificationControl workspace="nurse" scope={`${data.departmentId}:${data.route}`} events={soundEvents}/>;
  return <AppShell user={user} adminNotifications={notifications} demoClock={clock.enabled ? { nowIso: clock.now.toISOString(), simulated: clock.simulated } : undefined} workflowStatus={<div className="workspace-sound-status">{currentEvent && currentPhase ? <span><strong>{currentEvent.mealType.name}</strong> — {MEAL_PHASE_LABEL[currentPhase]}</span> : null}{soundControl}</div>}><main className="nurse-report-page">
