...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string; meal?: string }> }) {
  ...
  return <AppShell user={user}><main className="workspace serving-page serving-workspace"><Separator className="page-separator" aria-hidden="true"/>
    ...
    {data.events.length > 0 && <nav className="meal-timeline" aria-label="Chọn bữa ăn hôm nay">{data.events.map((event) => {
      ...
      const state = report ? "Đã gửi" : editable ? "Đang nhận" : "Đã chốt";
      return <a key={event.id} href={`/bao-suat?meal=${encodeURIComponent(event.id)}`} className={active ? "meal-timeline-item active" : "meal-timeline-item"} aria-current={active ? "page" : undefined}><span className={report ? "meal-state-dot sent" : editable ? "meal-state-dot open" : "meal-state-dot locked"}/><span><strong>{event.mealType.name}</strong><small>Chốt {event.mealType.cutoffTime}</small></span><em className={report ? "sent" : editable ? "open" : "locked"}>{state}</em></a>;
    })}</nav>}
    <div className="serving-event-list">{data.events.filter((event) => event.id === selectedEvent?.id).map((event) => {
      const report = event.reports[0];
      const byDiet = new Map(report?.lines.map((line) => [line.dietTypeId, line]));
      const editable = isBeforeCutoff(event.mealDate, event.mealType.cutoffTime);
      return <section className="serving-event" key={event.id}>
        <div className="serving-event-head"><div><p className="eyebrow">Bữa đang chọn</p><h2>{event.mealType.name} <span>· Chốt lúc {event.mealType.cutoffTime}</span></h2></div><span className={editable ? "cutoff-state open" : "cutoff-state locked"}>{editable ? "Đang nhận báo suất" : "Đã chốt số gốc"}</span></div>
