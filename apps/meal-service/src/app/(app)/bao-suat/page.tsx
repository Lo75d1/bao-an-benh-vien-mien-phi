...
export default async function ServingReportPage({ searchParams }: { searchParams: Promise<{ saved?: string }> }) {
  ...
  const editable = workflow?.phase === "REPORTING";
  const lateAdditionTrigger = event ? <Dialog><DialogTrigger asChild><button type="button" className="primary-action"><Plus aria-hidden="true"/>Báo bổ sung</button></DialogTrigger><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Báo bổ sung cho {event.mealType.name}</DialogTitle><DialogDescription>Sau giờ chốt chỉ bổ sung cho đúng bữa đang xử lý; hệ thống không chuyển sang bữa kế tiếp.</DialogDescription></DialogHeader><form action={addLateMealAction} className="nurse-late-addition-form"><input type="hidden" name="mealEventId" value={event.id}/><label>Mã chế độ ăn<select name="dietTypeId" required defaultValue=""><option value="" disabled>Chọn mã chế độ</option>{event.dietMeals.map((meal) => <option key={meal.dietTypeId} value={meal.dietTypeId}>{meal.dietType.code} · {meal.dietType.name}</option>)}</select></label><label>Số suất bổ sung<input type="number" name="quantity" min="1" step="1" inputMode="numeric" required/></label><label>Lý do bắt buộc<textarea name="reason" minLength={3} maxLength={500} required placeholder="Ví dụ: bệnh nhân mới nhập khoa sau giờ chốt"/></label><button type="submit" className="primary-action">Gửi bổ sung cho bếp</button></form></DialogContent></Dialog> : null;
