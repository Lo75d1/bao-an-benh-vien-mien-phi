...
import { localizeReportData, parseReportContent, parseReportFormat, parseReportRange, readReportBundle, type ReportData } from "@/lib/reports";
...
async function excel(report: ReportData, language: Language) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = language === "en" ? "Hospital Meal Service" : "Suất ăn bệnh viện";
  ...
  sections.forEach((section, index) => {
    ...
    sheet.addRow([language === "en" ? `From ${report.from} to ${report.to}` : `Từ ${report.from} đến ${report.to}`]); sheet.mergeCells(2, 1, 2, Math.max(section.columns.length, 1));
    sheet.addRow([`${language === "en" ? "Scope" : "Phạm vi"}: ${report.scope}`]); sheet.mergeCells(3, 1, 3, Math.max(section.columns.length, 1));
...
async function pdf(report: ReportData, language: Language) {
  ...
  document.fontSize(9).fillColor("#52645f").text(language === "en" ? `From ${report.from} to ${report.to} · Scope: ${report.scope}` : `Từ ${report.from} đến ${report.to} · Phạm vi: ${report.scope}`); document.moveDown();
  const sections = report.sections ?? [{ title: report.title, columns: report.columns, rows: report.rows }];
  for (const [sectionIndex, section] of sections.entries()) {
    ...
    if (!section.rows.length) document.fontSize(9).fillColor("#667772").text(language === "en" ? "—  No data in the selected range." : "—  Chưa có dữ liệu trong khoảng đã chọn.", 32, document.y + 8);
    document.moveDown();
  }
  document.end(); return done;
}

function printHtml(report: ReportData, language: Language) {
  const sections = report.sections ?? [{ title: report.title, columns: report.columns, rows: report.rows }];
  const content = sections.map((section) => { const headers = section.columns.map((column) => `<th>${safeHtml(column.label)}</th>`).join(""); const rows = section.rows.map((row) => `<tr>${section.columns.map((column) => `<td>${safeHtml(row[column.key])}</td>`).join("")}</tr>`).join(""); return `<section><h2>${safeHtml(section.title)}</h2>${rows ? `<table><thead><tr>${headers}</tr></thead><tbody>${rows}</tbody></table>` : `<p>${language === "en" ? "— No data in the selected range." : "— Chưa có dữ liệu trong khoảng đã chọn."}</p>`}</section>`; }).join("");
  const period = language === "en" ? `From ${report.from} to ${report.to} · Scope: ${safeHtml(report.scope)}` : `Từ ${report.from} đến ${report.to} · Phạm vi: ${safeHtml(report.scope)}`;
  return `<!doctype html><html lang="${language}"><head><meta charset="utf-8"><title>${safeHtml(report.title)}</title><style>body{font:13px Arial,sans-serif;color:#172b27;margin:28px}h1,h2{color:#123c36}h1{font-size:22px}h2{margin-top:24px;font-size:16px}p{color:#52645f}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{padding:7px;border:1px solid #cdd8d3;text-align:left}th{background:#edf5f2;color:#123c36}@media print{button{display:none}section{break-inside:avoid}}button{padding:10px 16px;background:#0f6e56;color:white;border:0;border-radius:6px}</style></head><body><button onclick="window.print()">${language === "en" ? "Print report" : "In báo cáo"}</button><h1>${safeHtml(report.title)}</h1><p>${period}</p>${content}<script>window.addEventListener('load',()=>window.print())</script></body></html>`;
}

export async function GET(request: Request) {
  const user = await getSessionUser();
  const language = normalizeLanguage(user?.language);
  if (!user) return new Response(language === "en" ? "Sign-in required." : "Cần đăng nhập.", { status: 401 });
  try {
    ...
    if (range.fromValue < settings.dataStartDate) return new Response(language === "en" ? `Data starts on ${settings.dataStartDate}.` : `Dữ liệu bắt đầu từ ${settings.dataStartDate}.`, { status: 400 });
    if (user.role === "NURSE" && contents.includes("warehouse")) return new Response(language === "en" ? "You do not have permission to export this content." : "Không có quyền xuất nội dung này.", { status: 403 });
    const report = localizeReportData(await readReportBundle(contents, range, user), language);
    if (format === "print") return new Response(printHtml(report, language), { headers: { "content-type": "text/html; charset=utf-8" } });
    const body = format === "excel" ? await excel(report, language) : await pdf(report, language);
  ...
  } catch { return new Response(language === "en" ? "Invalid report export request." : "Yêu cầu xuất báo cáo không hợp lệ.", { status: 400 }); }
}
