...
import { Check, FileDown, FileSpreadsheet, Printer, ShieldCheck } from "lucide-react";
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; content?: string; preview?: string }> }) {
  ...
  return <AppShell user={user}><main className="workspace report-page report-workbench"><Separator className="page-separator" aria-hidden="true"/>
    ...
    <div className="report-work-grid"><aside className="report-content-panel" aria-labelledby="report-builder-title"><header><span><ShieldCheck aria-hidden="true"/></span><div><h2 id="report-builder-title">Nội dung sẽ xuất</h2><p>Bộ đầy đủ đã chọn sẵn các dữ liệu cần đối chiếu.</p></div></header>
      <div className="report-content-list"><label><input type="checkbox" checked readOnly/><span><strong>Báo suất theo khoa</strong><small>Suất gốc, người báo, thời gian và trạng thái</small></span><Check aria-hidden="true"/></label><label><input type="checkbox" checked readOnly/><span><strong>Phát sinh sau chốt</strong><small>Số bổ sung, lý do và xác nhận của bếp</small></span><Check aria-hidden="true"/></label><label><input type="checkbox" checked readOnly/><span><strong>Thực đơn & dinh dưỡng</strong><small>Món, thực phẩm, kcal, khuyến nghị và đánh giá</small></span><Check aria-hidden="true"/></label><label><input type="checkbox" checked readOnly/><span><strong>Bằng chứng bếp</strong><small>Loại ảnh, ghi chú, người lưu và thời điểm</small></span><Check aria-hidden="true"/></label>{user.role !== "NURSE" && <label><input type="checkbox" checked readOnly/><span><strong>Dữ liệu kho</strong><small>Nhập, xuất, điều chỉnh và đơn vị ghi nhận</small></span><Check aria-hidden="true"/></label>}</div>
