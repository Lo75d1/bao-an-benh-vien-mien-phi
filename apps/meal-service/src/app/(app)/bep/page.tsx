...
import { KitchenMenuDetailDialog, type KitchenMenuItem } from "@/components/meal-detail-dialog";
...
function kitchenMenuItems(value: unknown): KitchenMenuItem[] {
  ...
  return value.items.flatMap((entry) => {
    ...
    if (typeof item.itemName !== "string" || !item.itemName.trim()) return [];
    return [{ name: item.itemName.trim(), dishName: typeof item.dishName === "string" && item.dishName.trim() ? item.dishName.trim() : "Món 1", grams: typeof item.grams === "number" && Number.isFinite(item.grams) ? item.grams : null, wastePercent: typeof item.wastePercent === "number" && Number.isFinite(item.wastePercent) ? item.wastePercent : null, note: typeof item.note === "string" && item.note.trim() ? item.note.trim() : null }];
  });
}
    ...
    {!meal ? <EmptyState icon={ChefHat} title="Không có dữ liệu bữa ăn" description="Hệ thống không tự tạo bữa hoặc đoán số suất."/> : <>
      ...
      <div className="kitchen-work-grid">
        <section className="kitchen-main-panel" aria-labelledby="serving-heading"><div className="compact-section-head"><div><p className="eyebrow">Số suất & tiến độ</p><h2 id="serving-heading">Theo chế độ ăn</h2></div><span className="tabular">{meal.dietMeals.length} chế độ</span></div>
          {meal.dietMeals.length === 0 ? <p className="panel-empty">— · Chưa có chế độ ăn cho bữa này.</p> : <div className="kitchen-table-scroll"><table className="kitchen-serving-table"><thead><tr><th scope="col">Chế độ ăn</th><th scope="col">Thực đơn</th><th scope="col">Suất gốc</th><th scope="col">Bổ sung</th><th scope="col">Tổng</th><th scope="col">Tiến độ</th><th scope="col"><span className="sr-only">Thao tác</span></th></tr></thead><tbody>{meal.dietMeals.map((item) => {
            const additions = meal.additions.filter((addition) => addition.dietTypeId === item.dietTypeId); const total = servingTotal(item.servingsPlanned, additions); const next = nextKitchenStatus(item.status); const menuItems = kitchenMenuItems(item.menuSnapshotJson); const menuNames = [...new Set(menuItems.map((menuItem) => menuItem.dishName))];
            return <tr key={item.id}><td><KitchenMenuDetailDialog code={item.dietType.code} name={item.dietType.name} servings={total.total} items={menuItems} notes={item.patientVisibleNote ? [item.patientVisibleNote] : []} trigger={<button type="button" className="kitchen-diet-link"><strong>{item.dietType.name}</strong><span><span translate="no">{item.dietType.code}</span> · {item.feedingRoute === "SONDE" ? "Sonde" : "Ăn thường"}</span></button>}/></td><td className="kitchen-menu-summary">{menuNames.length ? menuNames.join(" · ") : <span className="warning">— · Chưa có thực đơn duyệt</span>}</td><td className="numeric">{item.servingsPlanned > 0 ? numberFormat.format(item.servingsPlanned) : "—"}</td><td className="numeric">{total.additions > 0 ? `+${numberFormat.format(total.additions)}` : "—"}</td><td className="numeric total-cell">{item.servingsPlanned > 0 ? numberFormat.format(total.total) : "—"}</td><td><span className={`status status-${item.status.toLowerCase()}`}>{KITCHEN_STATUS_LABEL[item.status]}</span></td><td className="action-cell">{next ? <form action={transitionMealAction}><input type="hidden" name="dietMealId" value={item.id}/><input type="hidden" name="target" value={next}/><button className="compact-action">{actionLabel[next as keyof typeof actionLabel]}</button></form> : <span className="workflow-done">Đã hoàn tất</span>}</td></tr>;
