...
export async function reopenKitchenEventAction(formData: FormData) { const user = await requireKitchen(); await reopenKitchenEvent(String(formData.get("eventId") ?? ""), user); revalidatePath("/bep"); revalidatePath("/lich"); redirect("/bep?updated=reopened"); }
export async function acknowledgeKitchenNoteAction(formData: FormData) { const user = await requireKitchen(); const noteId = String(formData.get("noteId") ?? ""); const note = await prisma.patientNote.findFirst({ where: { id: noteId, status: "APPROVED" }, select: { id: true } }); if (!note) throw new Error("Không tìm thấy ghi chú đã duyệt."); await prisma.auditLog.create({ data: { entityType: "PatientNote", entityId: note.id, action: "KITCHEN_READ", actorId: user.id, actorName: user.displayName, reason: "Bếp xác nhận đã đọc ghi chú" } }); revalidatePath("/bep"); }
