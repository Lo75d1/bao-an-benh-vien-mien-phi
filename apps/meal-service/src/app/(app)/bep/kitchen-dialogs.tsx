...
export function KitchenDialogs({
  ...
  return (
    <div className="kitchen-tools" aria-label="Công cụ bữa đang chọn">
      <Dialog>
        <DialogTrigger asChild>
          <button type="button" className={pendingCount > 0 ? "tool-button attention" : "tool-button"}>
            <span>Suất bổ sung</span>
            <strong className="tabular">{pendingCount || "—"}</strong>
          </button>
        </DialogTrigger>
        <KitchenDialogShell
          title="Suất bổ sung & xác nhận"
          description="Xử lý từng phát sinh bằng đúng một trạng thái xác nhận của bếp."
          maxWidthClass="max-w-4xl"
        >
          {!canOperate ? <p className="kitchen-operation-locked">Chưa tới giờ chuẩn bị. Bếp chỉ được xem thông tin.</p> : null}
          {additions.length === 0 ? (
            <p className="dialog-empty">— · Không có suất bổ sung cho bữa này.</p>
          ) : (
                ...
                return (
                  <article className={item.kind === "URGENT_POST_SERVE" ? "urgent" : undefined} key={item.id}>
                    <div className="addition-summary">
                      <strong>+{item.quantity} suất · {item.dietType.name}</strong>
                      <span>{item.department.name} · {item.kind === "URGENT_POST_SERVE" ? "Khẩn sau phục vụ" : "Sau chốt"}</span>
                      <p>{item.reason}</p>
                    </div>
                    {item.ackStatus === "PENDING" ? (
                      <form action={acknowledgeAdditionAction}>
                        <input type="hidden" name="additionId" value={item.id} />
                        <label htmlFor={noteId}>Ghi chú bếp</label>
                        <input
                          id={noteId}
                          name="kitchenNote"
                          maxLength={500}
                      ...
                      <div className={`ack-result ack-${item.ackStatus.toLowerCase()}`}>
                        ...
                        <span>{item.kitchenNote ?? "Không có ghi chú bếp."}</span>
                      </div>
                    )}
                  </article>
                );
      ...
      <Dialog>
        ...
        <KitchenDialogShell
          ...
          description="Ảnh là tệp đính kèm, không làm thay đổi trạng thái bữa ăn."
          maxWidthClass="max-w-4xl"
        >
          ...
          <form className="evidence-dialog-form" action={uploadEvidenceAction} encType="multipart/form-data">
            ...
            <label>
              Ghi chú
              <input name="note" maxLength={500} autoComplete="off" placeholder="Nhập ghi chú nếu cần…" />
            </label>
          ...
          {visibleEvidence.length === 0 ? (
            <p className="dialog-empty">— · Chưa có bằng chứng hợp lệ để hiển thị.</p>
          ) : (
            <div className="evidence-dialog-list">
              {visibleEvidence.map((item) => (
                <article key={item.id}>
                  <img src={item.publicUrl!} alt={`${EVIDENCE_LABEL[item.kind]} · ${item.dietName}`} width="112" height="76" loading="lazy" />
                  <div>
                    <strong>{EVIDENCE_LABEL[item.kind]}</strong>
      ...
      <Dialog>
        <DialogTrigger asChild>
          <button type="button" className="tool-button">
            <span>Ghi chú đã xác nhận</span>
            <strong className="tabular">{patientNotes.length || "—"}</strong>
          </button>
        </DialogTrigger>
        <KitchenDialogShell
          title="Ghi chú bệnh nhân đã xác nhận"
          description="Chỉ hiển thị nội dung đã qua điều dưỡng xác nhận."
          maxWidthClass="max-w-3xl"
        >
          {!canOperate ? <p className="kitchen-operation-locked">Chưa tới giờ chuẩn bị. Bếp có thể đọc nhưng chưa xác nhận.</p> : null}
          {patientNotes.length === 0 ? (
            <p className="dialog-empty">— · Không có ghi chú đã xác nhận.</p>
          ) : (
                ...
                <article key={note.id}>
                  ...
                  {note.acknowledged ? (
                    ...
                    <form action={acknowledgeKitchenNoteAction}>
                      ...
                      <button disabled={!canOperate}>Xác nhận đã đọc</button>
                    </form>
                  )}
                </article>
              ))}
