...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ updated?: string; sync?: string; syncJob?: string }> }) {
  ...
  return <AppShell user={user}><main className="workspace admin-page admin-workspace"><Separator className="page-separator" aria-hidden="true"/>
    ...
    <section id="departments" className="admin-panel"><div className="section-heading"><div><p className="eyebrow">Khoa điều trị</p><h2>Danh mục khoa của bệnh viện</h2></div><span>{activeDepartments.length} đang dùng · {departments.length - activeDepartments.length} vô hiệu</span></div>
      <div className="admin-inline-create"><div><strong>Thêm khoa điều trị</strong><small>Tạo đủ khoa thực tế của bệnh viện; không giới hạn ở Nội và Ngoại.</small></div><form action={saveDepartmentAction} className="admin-grid department-create"><label>Mã khoa<input name="code" pattern="[A-Za-z0-9_-]{2,20}" autoComplete="off" spellCheck={false} required/></label><label>Tên khoa<input name="name" autoComplete="off" required/></label><button className="primary-action">Thêm khoa</button></form></div>
      <DepartmentTable saveAction={saveDepartmentAction} statusAction={departmentStatusAction} data={departments.map((department) => ({ id: department.id, code: department.code, name: department.name, status: department.status, statusLabel: department.status === "ACTIVE" ? "Đang dùng" : "Đã vô hiệu", publicUrl: department.status === "ACTIVE" ? buildPublicPatientUrl(settings.publicBaseUrl, department.publicToken) : null }))}/>
    </section>
    ...
    <section id="accounts" className="admin-panel"><div className="section-heading"><div><p className="eyebrow">Nhân sự & tài khoản</p><h2>Danh sách người dùng</h2></div><span>{activeUsers} hoạt động · {users.length - activeUsers} vô hiệu</span></div>
      <details className="admin-create-drawer"><summary><span><strong>Tạo tài khoản mới</strong><small>Chỉ mở biểu mẫu khi cần thêm nhân sự</small></span><em>Mở biểu mẫu</em></summary><div className="admin-create-body"><AccountCreateForm departments={activeDepartments} action={saveAccountAction}/></div></details>
      <AccountTable departments={activeDepartments} saveAction={saveAccountAction} statusAction={accountStatusAction} data={users.map((account) => ({ id: account.id, name: account.displayName, email: account.email, role: account.role, roleLabel: roleLabel[account.role], departmentId: account.memberships[0]?.departmentId ?? "", department: account.memberships[0]?.department.name ?? "—", kitchenRoute: account.kitchenRoute ?? "", kitchenScope: account.role === "KITCHEN" ? account.kitchenRoute === "SONDE" ? "Bếp Sonde" : "Bếp ăn thường" : "—", status: account.status, statusLabel: account.status === "ACTIVE" ? "Đang hoạt động" : "Đã vô hiệu" }))}/>
    </section>
