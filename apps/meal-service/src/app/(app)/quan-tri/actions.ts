...
async function saveSettings(formData: FormData) {
  ...
  await updateOperationalSettings({ dataStartDate: String(formData.get("dataStartDate") ?? ""), advanceEntryDays: Number(formData.get("advanceEntryDays")), publicMenuImages: enabled(formData, "publicMenuImages"), publicMenuDishes: enabled(formData, "publicMenuDishes"), publicMenuIngredients: enabled(formData, "publicMenuIngredients"), publicViewCountVisible: enabled(formData, "publicViewCountVisible"), foodRetention24hRequired: enabled(formData, "foodRetention24hRequired"), sondeEnabled: enabled(formData, "sondeEnabled"), warehouseMode: formData.get("warehouseMode") === "B" ? "B" : "A", warehouseApprovalRole: String(formData.get("warehouseApprovalRole")) as Role, serviceCompletionMinutes: Number(formData.get("serviceCompletionMinutes")), publicBaseUrl: String(formData.get("publicBaseUrl") ?? ""), patientSubmissionRecipients: { feedback: { dietitian: enabled(formData, "feedbackDietitian"), nurse: enabled(formData, "feedbackNurse") }, kitchenNote: { dietitian: enabled(formData, "kitchenNoteDietitian"), nurse: enabled(formData, "kitchenNoteNurse") } } }, ids.map((id, index) => ({ id, cutoffTime: cutoffs[index], serviceTime: services[index] })), actor, String(formData.get("reason") ?? ""));
