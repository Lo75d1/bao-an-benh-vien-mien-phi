...
const settingsSchema = z.object({ advanceEntryDays: z.number().int().min(1, "Tối thiểu 1 ngày.").max(60, "Tối đa 60 ngày."), serviceCompletionMinutes: z.number().int().min(15, "Tối thiểu 15 phút.").max(240, "Tối đa 240 phút."), publicMenuImages: z.boolean(), publicViewCountVisible: z.boolean(), foodRetention24hRequired: z.boolean(), sondeEnabled: z.boolean(), warehouseMode: z.enum(["A", "B"]), warehouseApprovalRole: z.enum(["ADMIN", "DIETITIAN", "KITCHEN"]), reason: z.string().trim().min(3, "Nêu lý do với ít nhất 3 ký tự.").max(500) });
...
export function SettingsForm({ settings, mealTypes, action }: { settings: Omit<SettingsFields, "reason">; mealTypes: MealType[]; action: (previous: ActionResult, data: FormData) => Promise<ActionResult> }) {
 ...
 return <form onSubmit={handleSubmit((values, event) => { const form = event?.currentTarget; if (form instanceof HTMLFormElement) { const data = new FormData(form); for (const key of ["sondeEnabled", "publicMenuImages", "publicViewCountVisible", "foodRetention24hRequired"] as const) data.set(key, values[key] ? "true" : "false"); startTransition(() => formAction(data)); } })} noValidate className="admin-form settings-form"><div className="admin-grid settings-primary-grid">
