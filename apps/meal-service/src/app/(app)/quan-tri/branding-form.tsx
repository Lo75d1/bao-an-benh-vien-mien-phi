...
export function BrandingForm({ branding, action }: { branding: BrandingSettings; action: (data: FormData) => Promise<void> }) {
  ...
  return <form action={submit} className="branding-direct-settings">
    ...
    <section className="brand-public-home" aria-labelledby="public-home-title">
      ...
      <div className={heroEnabled ? "brand-public-preview has-image" : "brand-public-preview"} style={{ "--preview-primary": publicPrimary, "--preview-accent": publicAccent, backgroundImage: heroEnabled ? `linear-gradient(90deg,rgba(248,251,253,.97),rgba(248,251,253,.2)),url("${heroImage ?? "/demo-hospital-meal-hero.png"}")` : undefined } as CSSProperties}><header><b>{shortName || "BV"}</b><strong>{name || "Tên bệnh viện"}</strong></header><div><span>Thực đơn dành cho người bệnh</span><h4>Xem thực đơn theo chế độ ăn</h4><button type="button">Xem thực đơn</button></div></div>
    </section>
