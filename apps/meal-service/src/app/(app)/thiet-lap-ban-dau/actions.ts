...
const languageOf = (formData: FormData) => normalizeLanguage(formData.get("language"));
const message = (error: unknown, language: Language) => error instanceof Error && error.message.trim() ? error.message : language === "en" ? "Unable to save changes." : "Không thể lưu thay đổi.";
function finish(step: number, language: Language, error?: unknown) {
  revalidatePath("/thiet-lap-ban-dau");
  redirect(`/thiet-lap-ban-dau?step=${step}&lang=${language}${error ? `&error=${encodeURIComponent(message(error, language))}` : ""}`);
}
...
export async function unlockBootstrapAction(formData: FormData) {
  ...
  catch (error) { finish(0, language, error); }
  finish(1, language);
}
...
export async function confirmAdminAction(formData: FormData) {
  ...
  catch (error) { finish(2, language, error); }
  finish(3, language);
}
...
export async function setupBrandingAction(formData: FormData) {
  ...
  } catch (error) { finish(1, language, error); }
  finish(2, language);
}
...
export async function setupDepartmentAction(formData: FormData) {
  ...
  catch (error) { finish(3, language, error); }
  finish(3, language);
}
...
export async function setupAccountCreateAction(language: Language, _previous: ActionResult, formData: FormData): Promise<ActionResult> {
  try {
    const id = await saveSetupAccount(formData);
    revalidatePath("/thiet-lap-ban-dau");
    return actionSuccess(id ? (language === "en" ? "Account updated." : "Đã cập nhật tài khoản.") : (language === "en" ? "Account created. The list has been updated." : "Đã tạo tài khoản. Danh sách đã được cập nhật."));
  } catch (error) {
...
export async function setupAccountAction(formData: FormData) {
  ...
  catch (error) { finish(4, language, error); }
  finish(4, language);
}
...
export async function setupRouteAction(formData: FormData) {
  ...
  } catch (error) { finish(5, language, error); }
  finish(6, language);
}
...
export async function setupDietTypeAction(formData: FormData) {
  ...
  catch (error) { finish(6, language, error); }
  finish(6, language);
}
...
export async function setupMealTypeAction(formData: FormData) {
  ...
  catch (error) { finish(7, language, error); }
  finish(7, language);
}
