...
export default async function ManagementPage({ searchParams }: { searchParams: Promise<{ date?: string; meal?: string; demoNow?: string; demoTour?: string }> }) {
  const user = await getSessionUser(); if (!user || !["ADMIN", "DIETITIAN"].includes(user.role)) redirect("/");
  const query = await searchParams;
  const [settings, clock] = await Promise.all([readOperationalSettings(), readRequestClock(query.demoNow, query.demoTour === "1" && Boolean(user.demoSessionId))]);
