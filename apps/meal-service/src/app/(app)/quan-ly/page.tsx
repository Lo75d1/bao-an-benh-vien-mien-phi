...
import { AdminVoiceNotifications } from "./admin-voice-notifications";
...
export default async function ManagementPage({ searchParams }: { searchParams: Promise<{ date?: string; meal?: string; demoNow?: string }> }) {
  ...
  const pendingVoiceEvents = dayData ? await prisma.lateMealAddition.findMany({ where: { mealEventId: { in: dayData.meals.map((meal) => meal.id) }, ackStatus: "PENDING" }, select: { id: true, mealEvent: { select: { mealType: { select: { feedingRoute: true } } } } } }) : [];
  ...
  return <AppShell user={user} adminNotifications={notifications} demoClock={clock.enabled ? { nowIso: clock.now.toISOString(), simulated: clock.simulated } : undefined} workflowStatus={<AdminVoiceNotifications events={pendingVoiceEvents.map((item) => ({ id: item.id, scope: item.mealEvent.mealType.feedingRoute }))}/>}><main className="management-page">{dayData ? <ManagementBoard data={dayData} dates={dates} initialMealTime={mealOverrideForClock(query.meal, clock.simulated)} role={user.role} liveClock={!clock.simulated}/> : <ErrorState title="Chưa tải được vận hành hôm nay" description="Không có dữ liệu nào được thay đổi."/>}</main></AppShell>;
}
