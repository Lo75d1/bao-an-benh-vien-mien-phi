...
export default async function ManagementPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  return <AppShell user={user} adminNotifications={notifications}><main className="management-page">{dayData ? <ManagementBoard data={dayData} dates={dates}/> : <ErrorState title="Chưa tải được quản lý suất ăn" description="Không có dữ liệu nào được thay đổi."/>}</main></AppShell>;
}
