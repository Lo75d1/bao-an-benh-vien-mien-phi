...
import { CalendarDays, ChevronRight, ClipboardPlus, Image as ImageIcon, LayoutList, Utensils } from "lucide-react";
...
function OperationalBadge({ status }: { status: { label: string; tone: string } }) {
  return <span className={`admin-operational-status status-${status.tone}`}>{status.label}</span>;
}
...
export function ManagementBoard({ data, dates, initialMealTime, role, liveClock = true }: { data: ManagementDay; dates: Array<{ value: string; label: string; active: boolean }>; initialMealTime?: string; role: Role; liveClock?: boolean }) {
  ...
  return <section className="admin-serving-board"><div className="admin-serving-top"><MealLifecycleStrip data={data} role={role} selectedMealId={meal.id} liveClock={liveClock}/><ReviewSchedule dates={dates} meals={data.meals} serviceCompletionMinutes={data.serviceCompletionMinutes} nowIso={data.generatedAt}/></div><div className="admin-serving-grid"><section className="admin-serving-master"><header><div><span>Vận hành toàn viện</span><h1>Vận hành hôm nay</h1></div><div className="admin-view-switch"><button className={mode === "department" ? "active" : ""} onClick={() => { setMode("department"); setSelectedId(""); }}><LayoutList/>Theo khoa</button><button className={mode === "diet" ? "active" : ""} onClick={() => { setMode("diet"); setSelectedId(""); }}><Utensils/>Theo mã chế độ ăn</button></div>{role === "ADMIN" ? <AddIncident meal={meal}/> : null}</header><div className="admin-serving-table"><table><thead><tr><th>{mode === "department" ? "Khoa" : "Mã chế độ ăn"}</th><th>Trạng thái</th><th>Số suất</th><th>Phát sinh</th><th/></tr></thead><tbody>{mode === "department" ? meal.departments.map((item) => { const additions = meal.additions.filter((addition) => addition.departmentId === item.id); return <tr className={activeId === item.id ? "selected" : ""} onClick={() => setSelectedId(item.id)} key={item.id}><th>{item.name}<small>{item.code}</small></th><td><OperationalBadge status={item.operationalStatus}/></td><td>{item.totalServings ?? "—"}</td><td>{additions.reduce((sum, value) => sum + value.quantity, 0) || "—"}</td><td><ChevronRight/></td></tr>; }) : meal.diets.map((item) => { const additions = meal.additions.filter((addition) => addition.dietCode === item.code); return <tr className={activeId === item.id ? "selected" : ""} onClick={() => setSelectedId(item.id)} key={item.id}><th>{item.code}<small>{item.name}</small></th><td><OperationalBadge status={item.operationalStatus}/></td><td>{item.servings ?? "—"}</td><td>{additions.reduce((sum, value) => sum + value.quantity, 0) || "—"}</td><td><ChevronRight/></td></tr>; })}</tbody><tfoot><tr><th>Tổng cộng</th><td>{mode === "department" ? `${meal.reportedDepartmentCount}/${meal.totalDepartmentCount}` : `${meal.diets.length} mã`}</td><td>{totals.servings ?? "—"}</td><td>{totals.additions || "—"}</td><td/></tr></tfoot></table></div></section>{mode === "department" ? <DepartmentDetail meal={meal} id={activeId}/> : <DietDetail meal={meal} diet={meal.diets.find((item) => item.id === activeId) ?? meal.diets[0]}/>}</div></section>;
}
