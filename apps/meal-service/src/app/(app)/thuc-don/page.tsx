...
import { formatDay } from "@/lib/presentation";
...
export default async function MenuPage({ searchParams }: { searchParams: Promise<{ meal?: string; saved?: string; route?: string }> }) {
  ...
  const message = language === "en"
    ? params.saved === "menus" || params.saved === "menu"
      ? "Menu saved. The system will lock automatically at cutoff."
      : params.saved === "template"
        ? "Personal template saved."
        : null
    : params.saved === "menus" || params.saved === "menu"
      ? "Đã lưu thực đơn. Hệ thống sẽ tự khóa khi tới giờ chốt."
      : params.saved === "template"
        ? "Đã lưu mẫu cá nhân."
        : null;
  return <AppShell user={user}><main className="nutrition-menu-page">
    ...
    <section className="nutrition-meal-context" aria-label={language === "en" ? "Meal being planned" : "Bữa đang lên thực đơn"} data-demo-guide="nutrition-picker">
      <div><CalendarDays aria-hidden="true"/><span><small>{language === "en" ? "Current meal" : "Bữa đang làm"} · {selected.feedingRoute === "SONDE" ? (language === "en" ? "Tube kitchen" : "Bếp Sonde") : (language === "en" ? "Oral kitchen" : "Bếp ăn thường")}</small><strong>{formatDay(selected.mealEvent.mealDate, language)} · {selected.mealEvent.mealType.name}</strong></span></div>
      <Dialog><DialogTrigger asChild><button type="button" className="secondary-button">{language === "en" ? "Choose meal" : "Chọn bữa"}</button></DialogTrigger><DialogContent className="nutrition-meal-dialog"><DialogHeader><DialogTitle>{language === "en" ? "Choose the meal and date to edit" : "Chọn ngày và bữa cần lên thực đơn"}</DialogTitle><DialogDescription>{language === "en" ? "Oral and tube-feeding schedules run independently. Pick one card to switch workspaces." : "Hai lịch ăn thường và Sonde vận hành độc lập. Chọn một ô để chuyển bàn làm việc."}</DialogDescription></DialogHeader><nav className="nutrition-route-tabs" aria-label={language === "en" ? "Choose feeding route" : "Chọn đường nuôi"}><Link href="/thuc-don?route=NORMAL" aria-current={selected.feedingRoute === "NORMAL" ? "page" : undefined}>{language === "en" ? "Oral kitchen" : "Bếp ăn thường"}</Link>{settings.sondeEnabled ? <Link href="/thuc-don?route=SONDE" aria-current={selected.feedingRoute === "SONDE" ? "page" : undefined}>{language === "en" ? "Tube kitchen" : "Bếp Sonde"}</Link> : null}</nav><div className="nutrition-meal-options">{eventGroups.filter((group) => group[0].feedingRoute === selected.feedingRoute).map((group) => {
        const first = group[0];
        ...
        return <Link key={first.mealEventId} href={`/thuc-don?meal=${encodeURIComponent(first.id)}`} aria-current={first.mealEventId === selected.mealEventId ? "true" : undefined}><span><strong>{formatDay(first.mealEvent.mealDate, language)}</strong><small>{first.mealEvent.mealType.name}</small></span><span className={complete ? "meal-picker-state complete" : "meal-picker-state missing"}>{complete ? <CheckCircle2 aria-hidden="true"/> : <CircleAlert aria-hidden="true"/>}{filled}/{group.length} {language === "en" ? "codes" : "mã"}</span><span className={locked ? "meal-picker-lock locked" : "meal-picker-lock"}>{locked ? <LockKeyhole aria-hidden="true"/> : null}{locked ? (language === "en" ? "Locked" : "Đã khóa") : (language === "en" ? "Editable" : "Còn sửa")}</span></Link>;
      })}</div></DialogContent></Dialog>
    </section>
    {!relatedMeals.length ? <EmptyState icon={Utensils} title={language === "en" ? "No diet codes for this meal" : "Bữa chưa có mã chế độ ăn"} description={language === "en" ? "Go back to the weekly calendar or ask an admin to add codes for this meal." : "Quay lại lịch tuần hoặc nhờ quản trị bổ sung mã cho bữa này."}/> : <MultiCodeMenuBoard
      context={{ eventId: selected.mealEventId, date: formatDay(selected.mealEvent.mealDate, language), mealName: selected.mealEvent.mealType.name, feedingRoute: selected.feedingRoute }} dataStartDate={settings.dataStartDate}
      language={language}
