...
export default async function MenuPage({ searchParams }: { searchParams: Promise<{ meal?: string; saved?: string; mode?: string }> }) {
  ...
  const [meals, foods, dishes, templates, warehouseNotes] = await Promise.all([
    ...
    prisma.dish.findMany({ where: { isActive: true, ingredients: { some: { foodId: { not: null } } } }, orderBy: { name: "asc" }, select: { id: true, name: true, totalWeightG: true, ingredients: { where: { foodId: { not: null } }, orderBy: { sortOrder: "asc" }, select: { quantityG: true, food: { select: { id: true, name: true, wastePercent: true, energyKcal: true, proteinG: true, lipidG: true, glucidG: true, sodiumMg: true, potassiumMg: true, waterG: true } } } } } }),
