...
export default async function MenuPage({ searchParams }: { searchParams: Promise<{ meal?: string; saved?: string }> }) {
  ...
  return <AppShell user={user}><main className="workspace menu-page">
    <PageHeader eyebrow="Bàn làm việc dinh dưỡng" title="Thực đơn theo bữa và chế độ ăn" description="Chọn việc cần xử lý, lập khẩu phần và kiểm tra chỉ tiêu trên cùng một màn hình." actions={<p className="scope-note"><strong>{meals.filter((meal) => !meal.approvedAt).length}</strong> thực đơn chưa duyệt</p>}/>
    ...
    {meals.length === 0 || !selected ? <EmptyState icon={Utensils} title="Chưa có bữa và chế độ ăn để lên thực đơn" description="Khởi tạo lịch tuần trước, sau đó quay lại màn này."/> : <>
      <Card className="menu-context-card"><CardContent><form className="menu-selector" method="get"><label>Việc đang xử lý<select name="meal" defaultValue={selected.id}>{meals.map((meal) => <option key={meal.id} value={meal.id}>{meal.approvedAt ? "✓" : "○"} {formatVnDay(meal.mealEvent.mealDate)} · {meal.mealEvent.mealType.name} · {meal.feedingRoute === "SONDE" ? "Sonde" : "Ăn thường"} · {meal.dietType.name} ({meal.dietType.code})</option>)}</select></label><button className="secondary-button">Chuyển việc</button></form>
