...
export default async function MenuPage({ searchParams }: { searchParams: Promise<{ meal?: string; saved?: string }> }) {
  ...
  const copyEventIds = [...new Set(meals.filter((meal) => meal.mealEventId !== selected.mealEventId && meal.feedingRoute === selected.feedingRoute && parseMenuItems(meal.menuSnapshotJson).length > 0).map((meal) => meal.mealEventId))].reverse().slice(0, 12);
