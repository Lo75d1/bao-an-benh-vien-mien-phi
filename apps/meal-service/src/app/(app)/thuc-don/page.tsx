...
import { approveMenuAction, approveMenusAction, deleteTemplateAction, saveTemplateAction } from "./actions";
...
export default async function MenuPage({ searchParams }: { searchParams: Promise<{ meal?: string; saved?: string; mode?: string }> }) {
  ...
  const message = params.saved === "approved" ? "Đã duyệt thực đơn và đóng băng snapshot." : params.saved === "approved-batch" ? "Đã duyệt hàng loạt; mỗi mã có snapshot và nhật ký riêng." : params.saved === "template" ? "Đã lưu mẫu cá nhân." : params.saved === "deleted" ? "Đã xóa mẫu chưa sử dụng." : null;
  return <AppShell user={user}><main className="workspace menu-page">
    ...
    {meals.length === 0 || !selected ? <EmptyState icon={Utensils} title="Chưa có bữa và chế độ ăn để lên thực đơn" description="Khởi tạo lịch tuần trước, sau đó quay lại màn này."/> : <>
      ...
      {mode === "single" ? <MenuEditor dietMeal={{ id: selected.id, dietTypeId: selected.dietTypeId, feedingRoute: selected.feedingRoute, approved: Boolean(selected.approvedAt), existing: parseMenuItems(selected.menuSnapshotJson) }} thresholds={thresholdsOf(selected.dietType.dietCodeRef)} templates={templates.map((template) => ({ id: template.id, label: template.name, items: template.items.map((item) => ({ foodId: item.foodId, itemName: item.itemName, grams: Number(item.grams), wastePercent: item.wastePercent === null ? null : Number(item.wastePercent) })) }))} copies={meals.filter((meal) => meal.id !== selected.id && meal.menuSnapshotJson !== null).map((meal) => ({ id: meal.id, label: `${formatVnDay(meal.mealEvent.mealDate)} · ${meal.mealEvent.mealType.name} · ${meal.dietType.name}`, items: parseMenuItems(meal.menuSnapshotJson) }))} approveAction={approveMenuAction} saveTemplateAction={saveTemplateAction}/> : <MultiCodeMenuBoard meals={relatedMeals.map((meal) => ({ id: meal.id, code: meal.dietType.code, name: meal.dietType.name, servings: meal.servingsPlanned, approved: Boolean(meal.approvedAt), thresholds: thresholdsOf(meal.dietType.dietCodeRef), items: parseMenuItems(meal.menuSnapshotJson).map((item) => ({ ...item, nutrients: { energyKcal: null, proteinG: null, lipidG: null, glucidG: null, sodiumMg: null, potassiumMg: null, waterG: null } })) }))} approveAction={approveMenusAction}/>}
