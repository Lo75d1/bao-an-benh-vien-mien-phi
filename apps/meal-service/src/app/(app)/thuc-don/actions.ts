...
export async function saveTemplateAction(formData: FormData) { const user = await requireDietitian(); if (user.demoSessionId) throw new Error("Demo Session không ghi mẫu cá nhân vào dữ liệu nền."); const dietMealId = String(formData.get("dietMealId") ?? ""); await createMenuTemplate({ name: String(formData.get("templateName") ?? ""), dietTypeId: String(formData.get("dietTypeId") ?? "") || null, feedingRoute: formData.get("feedingRoute") === "SONDE" ? "SONDE" : "NORMAL", items: readItems(formData) }, user); revalidatePath("/thuc-don"); redirect(`/thuc-don?meal=${encodeURIComponent(dietMealId)}&saved=template`); }
export async function deleteTemplateAction(formData: FormData) { const user = await requireDietitian(); if (user.demoSessionId) throw new Error("Demo Session không xóa mẫu trong dữ liệu nền."); await deleteMenuTemplate(String(formData.get("templateId") ?? ""), user); revalidatePath("/thuc-don"); redirect("/thuc-don?saved=deleted"); }
