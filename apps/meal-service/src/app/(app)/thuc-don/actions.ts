...
export async function approveMenusAction(formData: FormData) {
  const user = await requireDietitian();
  const raw = formData.get("menus");
  if (typeof raw !== "string") throw new Error("Dữ liệu duyệt hàng loạt không hợp lệ.");
  const menus = JSON.parse(raw) as Array<{ dietMealId: string; items: MenuItemInput[] }>;
  if (!Array.isArray(menus) || menus.length === 0) throw new Error("Chưa chọn mã nào để duyệt.");
  for (const menu of menus) await approveDietMeal({ dietMealId: menu.dietMealId, items: menu.items, sourceTemplateId: null }, user);
  revalidatePath("/thuc-don"); revalidatePath("/lich");
  redirect(`/thuc-don?mode=multiple&meal=${encodeURIComponent(menus[0].dietMealId)}&saved=approved-batch`);
}
