...
export default async function WarehousePage({ searchParams }: { searchParams: Promise<{ updated?: string; storage?: string }> }) {
 ...
 return <AppShell user={user}><main className="workspace warehouse-page"><PageHeader eyebrow="Kho nguyên liệu" title="Nhập, xuất và điều chỉnh" description="Ghi nhận giao dịch, chứng từ và đối chiếu dự kiến với thực xuất." actions={<p className="scope-note">Mode {data.mode}: {data.mode === "A" ? "một kho tổng" : "tách kho bếp và kho sonde theo đường nuôi"}</p>}/>
