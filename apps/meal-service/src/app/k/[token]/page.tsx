...
export default async function PatientPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ note?: string }> }) {
  ...
  return <main className="patient-page" id="noi-dung-chinh">
    <header className="patient-header"><p className="eyebrow">Suất ăn bệnh viện</p><h1 className="text-balance">{data.department.name}</h1><p>Thông tin bữa ăn theo khoa, không chứa hồ sơ người bệnh.</p></header>
    <section className="patient-section" aria-labelledby="current-meal">
      ...
      {!data.current ? <div className="patient-empty" role="status"><strong>—</strong><p>Chưa có dữ liệu bữa ăn đã báo cho khoa.</p></div> : <div className="public-diet-list">{data.current.dietMeals.map((meal) => <article className="public-diet" key={meal.id}>
        {meal.evidence[0]?.publicUrl ? <img src={meal.evidence[0].publicUrl} alt={`Ảnh bữa ăn ${meal.dietType.name}`} width={1200} height={900}/> : <div className="patient-photo-empty" role="img" aria-label={`Chưa có ảnh bữa ăn ${meal.dietType.name}`}><ImageOff aria-hidden="true"/><strong>—</strong><span>Chưa có ảnh bữa ăn</span></div>}
