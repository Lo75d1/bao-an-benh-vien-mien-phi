...
export async function handoffMealEvent(
  ...
  actor: { id: string; displayName: string; role: Role; kitchenRoute: FeedingRoute | null; demoSessionId?: string },
  now = new Date(),
) {
  ...
  if (actor.demoSessionId) {
    ...
    await updateDemoState((state) => {
      for (const snapshot of demoSnapshots) {
        state.handoffs = state.handoffs.filter((item) => !(item.mealEventId === event.id && item.departmentId === snapshot.departmentId));
        state.handoffs.push({ ...snapshot, mealEventId: event.id, handedOffAt, handedOffBy: actor.displayName });
      }
    });
    return demoSnapshots.map((snapshot) => ({ ...snapshot, mealEventId: event.id, handedOffAt: now, handedOffById: actor.id }));
  }
