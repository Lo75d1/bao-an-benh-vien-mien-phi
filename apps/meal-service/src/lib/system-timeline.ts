...
export async function synchronizeSystemTimeline(actor: { id: string; displayName: string }, now = new Date()) {
  ...
  for (const event of events) {
    ...
    if (!times) continue;
    const milestones = [{ action: "SYSTEM_PHASE_PREPARING", phase: "PREPARATION", occurredAt: times.cutoffAt }, { action: "SYSTEM_PHASE_SERVING", phase: "SERVICE", occurredAt: times.serviceAt }];
    for (const milestone of milestones) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (exists) return;
        await tx.auditLog.create({ data: { entityType: "MealEvent", entityId: event.id, action: milestone.action, actorId: actor.id, actorName: actor.displayName, afterJson: { phase: milestone.phase, occurredAt: milestone.occurredAt.toISOString(), source: "SYSTEM" }, reason: `Hệ thống ghi giai đoạn theo giờ cấu hình: ${milestone.phase}` } });
      });
    }
  }
}
