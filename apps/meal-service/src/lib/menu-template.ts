...
export async function createMenuTemplate(input: { name: string; dietTypeId?: string | null; feedingRoute?: "NORMAL" | "SONDE" | null; items: MenuItemInput[] }, actor: { id: string; displayName: string; language?: string | null }) {
  const language = normalizeLanguage(actor.language);
  const name = input.name.trim();
  if (!name || input.items.length === 0) throw new Error(language === "en" ? "A template name and at least one food are required." : "Tên mẫu và ít nhất một thực phẩm là bắt buộc.");
  const items = await normalizeMenuItems(input.items, language);
...
export async function deleteMenuTemplate(templateId: string, actor: { id: string; displayName: string; language?: string | null }) {
  const language = normalizeLanguage(actor.language);
  return prisma.$transaction(async (tx) => {
    ...
    if (!template) throw new Error(language === "en" ? "The personal template could not be found." : "Không tìm thấy mẫu cá nhân.");
    if (!canDeleteMenuTemplate(template._count.usedByMeals)) throw new Error(language === "en" ? "A template already used by a menu cannot be deleted." : "Không thể xóa mẫu đã được dùng cho thực đơn.");
