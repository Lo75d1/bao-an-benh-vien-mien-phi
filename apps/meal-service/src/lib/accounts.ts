import type { ActiveStatus, Prisma, Role } from "@prisma/client";
import { normalizeEmail, validPassword } from "./auth";
import { hashPassword } from "./password";
import { prisma } from "./prisma";

export type AccountInput = { email: unknown; displayName: unknown; role: unknown; password?: unknown; departmentId?: unknown };
...
export function validateAccountInput(input: AccountInput, requirePassword: boolean) {
  ...
  return { email, displayName, role, password: password || null, departmentId: role === "NURSE" ? departmentId : null };
}

async function requireAdmin(actor: { role: Role }) { if (actor.role !== "ADMIN") throw new Error("Chỉ quản trị viên được quản lý tài khoản."); }
const snapshot = (user: { email: string; displayName: string; role: Role; status: ActiveStatus }, departmentId: string | null) => ({ ...user, departmentId });
...
export async function createAccount(input: AccountInput, actor: { id: string; displayName: string; role: Role }) {
  ...
  return prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { email: data.email, displayName: data.displayName, role: data.role, passwordHash, memberships: data.departmentId ? { create: { departmentId: data.departmentId } } : undefined } });
...
export async function updateAccount(id: string, input: AccountInput, actor: { id: string; displayName: string; role: Role }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.user.update({ where: { id }, data: { email: data.email, displayName: data.displayName, role: data.role, ...(data.password ? { passwordHash: hashPassword(data.password) } : {}), memberships: data.departmentId ? { create: { departmentId: data.departmentId } } : undefined } });
