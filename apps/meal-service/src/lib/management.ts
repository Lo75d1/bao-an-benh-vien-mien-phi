...
export async function readManagementDay(date?: string, now = new Date(), departmentIds?: string[], feedingRoute?: FeedingRoute, configuredCompletionMinutes?: number): Promise<ManagementDay> {
  ...
  const servedLogs = dietMealIds.length === 0 ? [] : await prisma.auditLog.findMany({ where: { entityType: "DietMeal", entityId: { in: dietMealIds }, action: { in: ["KITCHEN_STATUS_CHANGE", "ADMIN_KITCHEN_MILESTONE_ADD"] } }, orderBy: { createdAt: "asc" }, select: { entityId: true, action: true, actorName: true, createdAt: true, afterJson: true } });
