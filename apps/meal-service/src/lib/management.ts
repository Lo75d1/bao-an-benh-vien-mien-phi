...
import { staffMealEvidenceUrl } from "./evidence-storage";
import { readOperationalSettings } from "./settings";
import { deriveOperationalStatus, getMealBusinessFacts, getMealPhase, type MealBusinessFacts, type MealPhase, type OperationalStatus } from "./meal-state";
...
export type ManagementDiet = { id: string; code: string; name: string; status: ManagementStatus; operationalStatus: OperationalStatus; servings: number | null; approved: boolean; menuItems: ManagementMenuItem[]; criteria: ManagementCriterion[]; approvedBy: string | null; reportedBy: string[]; kitchenLead: string | null; kitchenTimes: Partial<Record<ManagementStatus, string>>; kitchenTimeSources: Partial<Record<ManagementStatus, "KITCHEN" | "ADMIN">>; evidence: ManagementEvidence[] };
export type ManagementDepartment = { id: string; code: string; name: string; reportId: string | null; submittedAt: string | null; submittedBy: string | null; totalServings: number | null; operationalStatus: OperationalStatus; deliveryReceipt: { status: "FULL" | "SHORT"; expectedQuantity: number; receivedQuantity: number; note: string | null; confirmedAt: string; confirmedBy: string } | null; lines: Array<{ dietCode: string; dietName: string; quantity: number }> };
...
export async function readManagementDay(date?: string, now = new Date(), departmentIds?: string[], feedingRoute?: FeedingRoute, configuredCompletionMinutes?: number): Promise<ManagementDay> {
  ...
  const [settings, departments, events] = await Promise.all([
    ...
    prisma.mealEvent.findMany({
      ...
      select: {
        ...
        mealHandoffs: { where: departmentIds ? { departmentId: { in: departmentIds } } : undefined, select: { departmentId: true, quantity: true } },
  ...
  if (demo) for (const event of events) {
    ...
    for (const overlay of demo.state.handoffs.filter((item) => item.mealEventId === event.id && (!departmentIds || departmentIds.includes(item.departmentId)))) {
      const current = event.mealHandoffs.findIndex((item) => item.departmentId === overlay.departmentId);
      const handoff = { departmentId: overlay.departmentId, quantity: overlay.quantity };
      if (current >= 0) event.mealHandoffs.splice(current, 1, handoff); else event.mealHandoffs.push(handoff);
    }
  ...
  return { date: day.toISOString().slice(0, 10), generatedAt: now.toISOString(), isToday: selectedIsToday, serviceCompletionMinutes: configuredCompletionMinutes ?? settings.serviceCompletionMinutes, departmentCount: departments.length, meals: events.map((event) => {
    ...
    const departmentRows = departments.map((department) => {
      ...
      return { ...department, reportId: report?.id ?? null, submittedAt: report?.submittedAt?.toISOString() ?? null, submittedBy: report?.submittedBy.displayName ?? null, totalServings: report ? lines.reduce((sum, line) => sum + line.quantity, 0) : null, operationalStatus: deriveOperationalStatus({ hasReceipt: Boolean(receipt), hasHandoff: handoffByDepartment.has(department.id), kitchen: businessFacts.kitchen, report: report ? "SENT" : businessFacts.report === "PARTIAL" ? "PARTIAL" : "NOT_SENT" }), deliveryReceipt: receipt ? { status: receipt.status, expectedQuantity: receipt.expectedQuantity, receivedQuantity: receipt.receivedQuantity, note: receipt.note, confirmedAt: receipt.confirmedAt.toISOString(), confirmedBy: receipt.confirmedBy.displayName } : null, lines: lines.map((line) => ({ dietCode: line.dietType.code, dietName: line.dietType.name, quantity: line.quantity })) };
    });
    ...
    return { id: event.id, name: event.mealType.name, cutoffTime: event.mealType.cutoffTime, serviceTime: event.mealType.serviceTime, cutoffAt: cutoff === null ? null : new Date(cutoff).toISOString(), serviceAt: service === null ? null : new Date(service).toISOString(), totalDiets: event.dietMeals.length, unapprovedDiets: event.dietMeals.filter((meal) => managementMenuItems(meal.menuSnapshotJson).length === 0).length, plannedServings: event.dietMeals.length ? plannedTotal : null, inventoryEntryCount: event.dietMeals.reduce((sum, meal) => sum + (inventoryCountByDiet.get(meal.id) ?? 0), 0), statusCounts, diets: event.dietMeals.map((meal) => {
      const reporters = event.reports.filter((report) => report.lines.some((line) => line.dietType.code === meal.dietType.code && line.quantity > 0));
      const reportFact = reporters.length === 0 ? "NOT_SENT" : departments.length > 0 && reporters.length < departments.length ? "PARTIAL" : "SENT";
      return { id: meal.id, code: meal.dietType.code, name: meal.dietType.name, status: meal.status as ManagementStatus, operationalStatus: deriveOperationalStatus({ hasReceipt: event.deliveryReceipts.length > 0, hasHandoff: event.mealHandoffs.length > 0, kitchen: getMealBusinessFacts({ dietStatuses: [meal.status], reportedDepartmentCount: reporters.length, totalDepartmentCount: departments.length }).kitchen, report: reportFact }), servings: meal.servingsPlanned > 0 ? meal.servingsPlanned : null, approved: mealTimePhase(day, event.mealType.cutoffTime, event.mealType.serviceTime, now) !== "BEFORE_CUTOFF", menuItems: managementMenuItems(meal.menuSnapshotJson), criteria: managementCriteria(meal.evaluationJson), approvedBy: meal.approvedBy?.displayName ?? null, reportedBy: [...new Set(reporters.map((report) => report.submittedBy.displayName))], kitchenLead: kitchenLeadByDiet.get(meal.id) ?? null, kitchenTimes: kitchenTimesByDiet.get(meal.id) ?? {}, kitchenTimeSources: kitchenTimeSourcesByDiet.get(meal.id) ?? {}, evidence: meal.evidence.map((item) => ({ id: item.id, kind: item.kind as ManagementEvidence["kind"], note: item.note, uploadedAt: item.uploadedAt.toISOString(), uploadedBy: item.uploadedBy.displayName, publicUrl: item.storagePath ? staffMealEvidenceUrl(item.id) : null })) };
    }), eventEvidence: event.evidence.map((item) => ({ id: item.id, kind: item.kind as ManagementEvidence["kind"], note: item.note, uploadedAt: item.uploadedAt.toISOString(), uploadedBy: item.uploadedBy.displayName, publicUrl: item.storagePath ? staffMealEvidenceUrl(item.id) : null })), foodRetention24hRequired: settings.foodRetention24hRequired, departments: departmentRows, additions: event.additions.map((item) => ({ id: item.id, departmentId: item.departmentId, departmentName: item.department.name, dietCode: item.dietType.code, dietName: item.dietType.name, quantity: item.quantity, reason: item.reason, ackStatus: item.ackStatus, submittedAt: item.submittedAt.toISOString(), submittedBy: item.submittedBy.displayName })), reportedDepartmentCount: submitted.length, totalDepartmentCount: departments.length, deliveryReceiptCount: event.deliveryReceipts.length, reportedServings: submitted.length > 0 ? submitted.reduce((sum, department) => sum + (department.totalServings ?? 0), 0) : null, businessFacts };
  }) };
}
