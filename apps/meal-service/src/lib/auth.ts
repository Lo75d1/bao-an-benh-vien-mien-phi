...
export type SessionUser = { id: string; email: string; displayName: string; role: Role; kitchenRoute: FeedingRoute | null; demoSessionId?: string; demoWorkspace?: import("./demo-session").DemoWorkspace };
...
export async function getSessionUser(): Promise<SessionUser | null> {
  const { demoWorkspaceIdentity, readDemoSession } = await import("./demo-session");
  const demo = await readDemoSession();
  if (demo) {
    const identity = demoWorkspaceIdentity(demo.workspace);
    const user = await prisma.user.findUnique({ where: { email: identity.email }, select: { id: true, email: true, displayName: true, status: true } });
    if (!user || user.status !== "ACTIVE") return null;
    return { id: user.id, email: user.email, displayName: user.displayName, role: identity.role, kitchenRoute: identity.kitchenRoute, demoSessionId: demo.id, demoWorkspace: demo.workspace };
  }
  const token = (await cookies()).get(SESSION_COOKIE)?.value; if (!token) return null;
  const session = await prisma.session.findUnique({ where: { tokenHash: hashToken(token) }, include: { user: { select: { id: true, email: true, displayName: true, role: true, kitchenRoute: true, status: true } } } });
  if (!session || session.expiresAt <= new Date() || session.user.status !== "ACTIVE") return null; return session.user;
}
