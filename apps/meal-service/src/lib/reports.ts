...
async function readDailyOverview(range: ReturnType<typeof parseReportRange>, servings: ReportSection, additions: ReportSection): Promise<{ overview: ReportSection; shopping: ReportSection }> {
  const columns = [{ key: "date", label: "Ngày" }, { key: "base", label: "Suất gốc" }, { key: "additions", label: "Phát sinh bếp nhận" }, { key: "total", label: "Tổng phục vụ" }, { key: "departments", label: "Theo khoa" }, { key: "diets", label: "Theo mã chế độ" }];
  const shoppingColumns = [{ key: "date", label: "Ngày" }, { key: "food", label: "Thực phẩm" }, { key: "edibleKg", label: "Ăn được (kg)" }, { key: "waste", label: "Thải bỏ" }, { key: "purchaseKg", label: "Cần mua (kg)" }];
  ...
  const shoppingRows = [...shoppingByFood.values()].sort((a, b) => a.date.localeCompare(b.date) || a.food.localeCompare(b.food, "vi")).map((item) => ({ date: item.date, food: item.food, edibleKg: Number((item.edibleGrams / 1000).toFixed(3)), waste: item.wastePercent === null ? null : `${item.wastePercent}%`, purchaseKg: item.rawGrams === null ? null : Number((item.rawGrams / 1000).toFixed(3)) }));
  return { overview: { title: "Tổng quan theo ngày", columns, rows: normalizeReportRows(rows, columns) }, shopping: { title: "Bảng đi chợ theo ngày", columns: shoppingColumns, rows: normalizeReportRows(shoppingRows, shoppingColumns) } };
}
...
export async function readReport(content: ReportContent, range: ReturnType<typeof parseReportRange>, actor: ReportActor): Promise<ReportData> {
  ...
  if (content === "menus") {
    const columns = [{ key: "date", label: "Ngày" }, { key: "meal", label: "Bữa" }, { key: "diet", label: "Mã chế độ" }, { key: "dishes", label: "Món ăn" }, { key: "foods", label: "Thực phẩm" }, { key: "energy", label: "kcal/suất" }, { key: "target", label: "Khuyến nghị" }, { key: "overall", label: "Đánh giá" }, { key: "approvedBy", label: "Người lập" }];
  ...
  if (content === "evidence") {
    ...
    const rows = evidence.flatMap((item) => { const event = item.mealEvent ?? item.dietMeal?.mealEvent; if (!event) return []; return [{ date: dateCell(event.mealDate), meal: event.mealType.name, diet: item.dietMeal ? `${item.dietMeal.dietType.code} — ${item.dietMeal.dietType.name}` : "Toàn bữa", kind: item.kind === "MEAL_PHOTO" ? "Ảnh món thực tế" : item.kind === "FOOD_SAMPLE" ? "Mẫu lưu thực phẩm 24 giờ" : item.kind, status: "Đã lưu", note: item.note, uploadedBy: item.uploadedBy.displayName, uploadedAt: dateTimeCell(item.uploadedAt) }]; });
    return { ...base, columns, rows: normalizeReportRows(rows, columns) };
  }
